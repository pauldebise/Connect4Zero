...
def async_learner_process(shared_queue, config, model_config):
    ...
    # On calcule une estimation de steps pour un cycle complet du buffer
    ...
    while True:
        ...
        # 1. On mémorise la taille actuelle du buffer
        ...
        for _ in range(config.train_steps_per_cycle):
            ...
            # CORRECTION : on assigne bien le retour à "logs"
            logs = global_model.model.train_on_batch(
                x=states, y={'policy_head': probs, 'value_head': values}, return_dict=True
            )
            step_counter += 1

        # 3. Le Frein (Throttling) calculé dynamiquement
        # Calcul des états consommés (ex: 10 steps * 256 batch_size = 2560 états vus)
        states_processed = config.train_steps_per_cycle * config.batch_size

        # Calcul des nouveaux états à attendre pour respecter le Replay Ratio
        # (ex: 2560 / 20.0 = 128 nouveaux états à attendre)
        states_to_wait = int(states_processed / config.target_replay_ratio)

        # Le GPU se met en veille tant que les CPU n'ont pas produit ce quota
        while len(replay_buffer) < current_buffer_size + states_to_wait:
            time.sleep(0.5)
        ...
        if step_counter % config.checkpoint_interval == 0:
            print(f"\n💾 [Checkpoint] Sauvegarde des poids et export ONNX au step {step_counter}...")

            # Sauvegarde des poids au format demandé
            global_model.model.save(weights_path)

            # Exportation propre (pour ne pas crasher les workers qui liraient en même temps)
            temp_onnx = 'temp_model.onnx'
            export_keras_to_onnx(global_model.model, temp_onnx)
            os.replace(temp_onnx, onnx_path)

            # Archivage pour le suivi de l'Elo
